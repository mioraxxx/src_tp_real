        ...
        <div class="card">
            <?php if ($success) { ?>
                <p style="color:green;">Enregistré.
                    <a href="fiche.php?emp_no=<?= urlencode($emp_no) ?>" class="alert-success">Voir la fiche &rarr;</a>
                </p>
            <?php } ?>
            <?php if ($error !== '') { ?>
                <p style="color:red;" class="alert-error"><?= htmlspecialchars($error) ?></p>
            <?php } ?>

            <form method="post" action="emp_form.php<?= $editing ? '?emp_no=' . urlencode($emp_no) : '' ?>" class="form-group label">
                <input class="form-control" type="hidden" name="mode" value="<?= $editing ? 'edit' : 'add' ?>">
